...
Rank: #25
Source: https://www.kaggle.com/c/pii-detection-removal-from-educational-data/discussion/497220
...
Therefore, I tried using ``Mistral-7b`` for few-shot generation to determine whether the content predicted to be the label ``name student`` is actually a name. (Here we cannot expect the model to distinguish whether it is a student's name or not, but only to exclude predictions that are clearly not names).
